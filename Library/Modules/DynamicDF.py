...
class DynamicDF(tf.Module):
    """
    Apply the Dynamic Dictionary Filter algorithm with Gaussian distribution model.
    ...
    def __init__(self, m, r, init_scale, train_lambda, name=None):
        ...
        self.V = tf.Variable(tf.multiply(tf.eye(r, dtype=tf.float32), 1000))
        self.Q = tf.Variable(tf.multiply(tf.eye(r, dtype=tf.float32), 5))
    ...
    def __call__(self, Y, return_V_updates=False):
        ...
        # Update C and V
        self.V.assign(tf.add(self.V, self.Q)) # shape: (r, r)
